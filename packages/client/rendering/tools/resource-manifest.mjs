...
export async function loadResourceManifest(dataRoot, manifestPath) {
  ...
  if (manifest.formatVersion !== 4) throw new Error("Unsupported client resource pack source format");
  ...
  const catalogs = await Promise.all(catalogFiles.map(async (file) => {
    ...
    const source = requireRecord(parse(await readFile(path, "utf8")), `texture catalog ${file}`);
    const category = requireText(source.category, `texture catalog ${file} category`);
    if (!/^[a-z][a-z0-9-]*$/u.test(category)) throw new Error(`Texture catalog category ${category} is invalid`);
    const textures = requireList(source.textures, `texture catalog ${file} textures`).map((texture) => ({
      ...requireRecord(texture, `texture catalog ${file} entry`),
      category,
    }));
    if (textures.length === 0) throw new Error(`Texture catalog ${file} is empty`);
    return {file, category, textures};
  }));
  ...
  for (const texture of textures) {
    ...
    if (!file.startsWith(`textures/${texture.category}/`) || !file.endsWith(".png")) {
