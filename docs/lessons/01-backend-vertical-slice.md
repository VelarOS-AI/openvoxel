...
1. 从 `packages/world/model/src/spatial/coordinates.vel` 理解负坐标和三维 Chunk。
2. 从 `packages/world/model/src/chunk/chunk.vel` 理解 UInt32 世界运行时 ID 调色板、定长 `UInt16Buffer` 局部索引和内存布局。
